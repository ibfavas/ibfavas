"""
Convert Mohammad Favas S portrait photo into a monochrome ASCII-art SVG terminal animation.
"""
...
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "favas-prepped.png")
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "favas-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">favas@github: ~$ ./mohammad-favas.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'favas@github:~$ whoami <tspan fill="{INK}">Mohammad Favas S</tspan></text>')
